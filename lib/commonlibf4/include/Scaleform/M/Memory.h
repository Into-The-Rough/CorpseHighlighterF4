...
namespace Scaleform
{
	...
	public:
		...
		{
			static REL::Relocation<MemoryHeap**> heap{
				RE::ID::Runtime(939898, ID::Memory::SetGlobalHeap)
			};
			*heap = a_heap;
		}
		...
		{
			static REL::Relocation<MemoryHeap**> heap{
				RE::ID::Runtime(939898, ID::Memory::GetGlobalHeap)
			};
			return *heap;
		}
...
#define SF_HEAP_REDEFINE_NEW(a_type)                                                                                    \
	...
	{                                                                                                                   \
		const auto mem = Scaleform::aligned_alloc(static_cast<std::size_t>(a_alignment), a_count);                      \
		if (mem) {                                                                                                      \
			return mem;                                                                                                 \
		} else {                                                                                                        \
			REX::FAIL("out of memory"sv);                                                                               \
	...
	[[nodiscard]] constexpr void* operator new[](std::size_t, std::align_val_t, void* a_ptr) noexcept { return a_ptr; } \
                                                                                                                        \
	void operator delete(void* a_ptr) { Scaleform::free(a_ptr); }                                                       \
	void operator delete[](void* a_ptr) { Scaleform::free(a_ptr); }                                                     \
	void operator delete(void* a_ptr, std::align_val_t) { Scaleform::free(a_ptr); }                                     \
	void operator delete[](void* a_ptr, std::align_val_t) { Scaleform::free(a_ptr); }                                   \
	void operator delete(void* a_ptr, std::size_t) { Scaleform::free(a_ptr); }                                          \
	void operator delete[](void* a_ptr, std::size_t) { Scaleform::free(a_ptr); }                                        \
	void operator delete(void* a_ptr, std::size_t, std::align_val_t) { Scaleform::free(a_ptr); }                        \
	void operator delete[](void* a_ptr, std::size_t, std::align_val_t) { Scaleform::free(a_ptr); }
