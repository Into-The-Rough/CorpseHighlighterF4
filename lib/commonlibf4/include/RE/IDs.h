...
namespace RE::ID
{
	inline bool IsOldGen() noexcept
	{
		static const bool oldGen =
			REX::FModule::GetExecutingModule().GetFileVersion() == REL::Version{ 1, 10, 163, 0 };
		return oldGen;
	}

	inline REL::ID Runtime(std::uint64_t a_oldGen, REL::ID a_current) noexcept
	{
		return REL::ID(IsOldGen() ? a_oldGen : a_current.id());
	}
